...
```text
You are helping with BoatBoard, a local-first visual company directory prototype. The repository is the source of truth; do not rely on old chat memory for important project behavior.
...
Inspect actual source files and Git state before editing. If the repo still has template placeholders, adapt it from verified files and the facts I provide; ask only for material unknowns. During setup, before serious implementation, ask me for the intended Git `user.name` and the one `user.email` verified/associated with the GitHub account that should receive attribution. After Git is initialized, immediately configure those clone-local settings, create `.git-identity` from its example using that email, and enable `.githooks`. Keep code explicit, modular, and easy to review.
